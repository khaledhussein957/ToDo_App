...
export const updatePasswordController = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(currentUser.userId, { password: hashedNewPassword }, { new: true });
        if (!updatedUser) {
            return res.status(404).json({ success: false, message: "User not found" });
        }
        ...
        res.status(200).json({ success: true, message: "Password updated successfully", user: userResponse });
